...
class Environment:
    ...
    def try_quit_driver(self):
        try:
            ...
            logger = get_logger()
            logger.debug('during quitting browse have got an exception {}'.format(exception))
    ...
    def start(self, url, user_info = None, payment_info = None):
        ...
        try:
            ...
            exception = traceback.format_exc()
            logger = get_logger()
            logger.debug('during starting url {} have got an exception {}'.format(url, exception))
    ...
    def get_next_control_based_frame(self, url, c_idx, f_idx):
    ...
    def try_switch_to_frame(self):
        try:
            ...
            exception = traceback.format_exc()
            logger = get_logger()
            logger.debug('during switching to frame have got an exception {}'.format(exception))
    ...
    def apply_action(self, control, action):
        ...
        try:
            ...
            exception = traceback.format_exc()
            logger = get_logger()
            logger.debug('during applying action have got an exception {}'.format(exception))
